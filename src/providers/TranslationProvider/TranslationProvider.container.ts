...
import { MapDispatchProps, MapStateProps, OwnProps } from './TranslationProvider.types'
...
const mapState = (state: any, ownProps: OwnProps): MapStateProps => {
  ...
  if (!isLoading(state)) {
    locale = getLocale(state) || getPreferredLocale(ownProps.locales) || ownProps.locales[0]
    if (locale) {
...
const mapDispatch = (dispatch: RootDispatch): MapDispatchProps => ({
  onFetchTranslations: (locale: Locale) => dispatch(fetchTranslationsRequest(locale))
})
