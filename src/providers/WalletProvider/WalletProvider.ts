...
import { AccountsChangedHandler, EmitterMethod, EventType, Handler, NetworkChangedHandler, Props } from './WalletProvider.types'

export default class WalletProvider extends React.PureComponent<Props> {
  handleChangeAccount = async () => {
    const { isConnected, isConnecting, address, onChangeAccount, appChainId } = this.props
    try {
  ...
  handleChangeNetwork = async () => {
    const { isConnected, isConnecting, chainId, onChangeNetwork, appChainId } = this.props
    try {
      const wallet = await buildWallet(appChainId)
