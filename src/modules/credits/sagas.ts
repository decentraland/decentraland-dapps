...
import { call, cancel, cancelled, delay, fork, put, race, select, take, takeEvery } from 'redux-saga/effects'
...
export function* creditsSaga(options: { creditsClient: CreditsClient }) {
  ...
  function* handleFetchCreditsRequest(action: FetchCreditsRequestAction) {
    ...
    const isCreditsEnabled: boolean = yield select(isCreditsFeatureEnabled, address)
    ...
    try {
      const credits: CreditsResponse = yield call([options.creditsClient, 'fetchCredits'], address)
      yield put(fetchCreditsSuccess(address, credits))
    } catch (error) {
      yield put(fetchCreditsFailure(address, isErrorWithMessage(error) ? error.message : 'Unknown error'))
    }
  }
  ...
  function* handleStartSSEConnection(action: StartCreditsSSEAction): Generator<any, void, any> {
    const { address } = action.payload
  ...
  function createCreditsEventChannel(address: string, isCreditsEnabled: boolean): EventChannel<any> {
  ...
  function* startSSEConnection(address: string): Generator<any, void, any> {
    ...
    try {
      ...
      const channel = yield call(createCreditsEventChannel, address, isCreditsEnabled)
  ...
  yield takeEvery(STOP_CREDITS_SSE, function* handleStopSSEConnection(): Generator<any, void, any> {
    if (sseConnectionTask) {
      yield cancel(sseConnectionTask)
