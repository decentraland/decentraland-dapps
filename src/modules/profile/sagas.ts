...
import { CHANGE_ACCOUNT, CONNECT_WALLET_SUCCESS, ChangeAccountAction, ConnectWalletSuccessAction } from '../wallet/actions'
import {
...
export function createProfileSaga({ getIdentity, peerUrl, peerWithNoGbCollectorUrl }: CreateProfileSagaOptions) {
  ...
  function* handleLoadProfilesRequest(action: LoadProfilesRequestAction) {
    ...
    try {
      const profiles: Profile[] = yield call([peerApi, 'fetchProfiles'], addresses)
  ...
  function* handleSetAlias(action: SetProfileAvatarAliasRequestAction) {
    ...
    try {
      const newAvatar: Avatar = yield updateProfileAvatarWithoutNewFiles(address, { hasClaimedName: true, name: alias })
   ...
   */
  function* handleSetProfileDescription(action: SetProfileAvatarDescriptionRequestAction) {
    try {
      const { address, description } = action.payload

      const newAvatar: Avatar = yield updateProfileAvatarWithoutNewFiles(address, { description: description })
