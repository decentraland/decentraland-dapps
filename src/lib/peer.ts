...
export class PeerAPI extends BaseAPI {
   ...
   */
  public async fetchProfile(address: string, options: FetchProfileOptions = { useCache: true }): Promise<Profile> {
    if (options.useCache && address in this.cache) {
      return this.cache[address]
    }

    this.cache[address] = this.lambdasClient.getAvatarsDetailsByPost({ ids: [address.toLowerCase()] }).then(profiles => profiles[0] as any)
  ...
  public async getDefaultProfileEntity(): Promise<ProfileEntity> {
    const profile = await this.request('GET', '/content/entities/profile?pointer=default' + Math.floor(Math.random() * 128 + 1))
    return profile[0]
  }
}
