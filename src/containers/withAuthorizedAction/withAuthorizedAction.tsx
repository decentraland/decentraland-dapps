...
const mapState = (state: RootStateOrAny): MapStateProps => ({
  isMagicAutoSignEnabled: getIsFeatureEnabled(state, ApplicationName.DAPPS, FeatureName.MAGIC_AUTO_SIGN),
...
const mapDispatch = (dispatch: MapDispatch): MapDispatchProps => ({
  ...
  onRevoke: (traceId: string, authorization: Authorization, onAuthorized?: () => void) =>
    dispatch(
...
export default function withAuthorizedAction<P extends WithAuthorizedActionProps>(
  ...
  const WithAuthorizedActionComponent = (props: MapStateProps & MapDispatchProps & any) => {
    const {
      authorizerWallet,
    ...
    const [authModalData, setAuthModalData] = useState<Omit<AuthorizationModalOwnProps, 'onGrant' | 'onRevoke'>>()
    const [isLoadingAuthorization, setIsLoadingAuthorization] = useState(false)
    const isUserLoggedInWithMagic = authorizerWallet && isWeb2Wallet(authorizerWallet)
    ...
    const handleRevoke = useCallback(
      (authorization: Authorization, analyticsTraceId: string, onAuthorized?: () => void) => {
    ...
    const handleAuthorizedAction = useCallback(
      async (authorizeOptions: AuthorizeActionOptions) => {
        ...
        const { authorizationType, targetContract, authorizedAddress, targetContractName, authorizedContractLabel, onAuthorized } =
          authorizeOptions
        ...
        try {
          if (authorizationType === AuthorizationType.ALLOWANCE) {
            ...
            const currentAllowance: BigNumber = await contract.allowance(userAddress, authorizedAddress)
            ...
            if (isMagicAutoSignEnabled && isUserLoggedInWithMagic && !authorizeOptions.manual) {
      ...
      [userAddress, isMagicAutoSignEnabled, isUserLoggedInWithMagic, handleGrant, handleRevoke]
    )
  ...
  return connect(mapState, mapDispatch)(WithAuthorizedActionComponent) as React.ComponentType<Omit<P, keyof WithAuthorizedActionProps>>
}
